...
function buildPrompt({ textStyle, hashtagCount, language, characterStyle, requiredKeyword }) {
  ...
  // 数値として処理（0も有効な値として扱う）
  const parsedCount = parseInt(hashtagCount);
  const count = isNaN(parsedCount) ? 3 : parsedCount;
...
【重要な制約】
- ハッシュタグは必ず正確に${count}個だけ生成してください
- ${count}個より多くても少なくてもいけません
- ハッシュタグは半角スペースで区切ってください
- 各ハッシュタグは#で始めてください
...
【ハッシュタグ】
（${hashtagLanguage}を#付きで正確に${count}個だけ記載。例: #タグ1 #タグ2 #タグ3）
`.trim();
}

// ハッシュタグ数を調整する関数
function adjustHashtagCount(hashtagsText, targetCount) {
  // ハッシュタグを抽出（#で始まる単語）
  const hashtagPattern = /#[\w\u3040-\u309F\u30A0-\u30FF\u4E00-\u9FFF\u3400-\u4DBF]+/g;
  const hashtags = hashtagsText.match(hashtagPattern) || [];

  console.log(`ハッシュタグ調整: 現在${hashtags.length}個 → 目標${targetCount}個`);

  if (hashtags.length === targetCount) {
    // 既に正しい数の場合はそのまま返す
    return hashtagsText;
  } else if (hashtags.length > targetCount) {
    // 多すぎる場合は先頭から指定数だけ取得
    const adjustedHashtags = hashtags.slice(0, targetCount);
    console.log(`ハッシュタグを${hashtags.length}個から${targetCount}個に削減しました`);
    return adjustedHashtags.join(' ');
  } else {
    // 少なすぎる場合はそのまま返す（AIが生成したものを尊重）
    console.warn(`ハッシュタグが${hashtags.length}個しかありません（目標: ${targetCount}個）`);
    return hashtagsText;
  }
}
...
async function generateCaption({ base64Image, textStyle, hashtagCount, language, characterStyle, requiredKeyword }) {
  ...
  try {
    ...
    // ハッシュタグ数を調整（指定数に合わせる、0も有効な値として扱う）
    const parsedHashtagCount = parseInt(hashtagCount);
    const targetCount = isNaN(parsedHashtagCount) ? 3 : parsedHashtagCount;
    hashtags = adjustHashtagCount(hashtags, targetCount);
